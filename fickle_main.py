...
def end_message(end_m):
    ...
    if 0 <= perc <= 29:
        text_res = emoji.emojize('''Все мы, как редуцированные, падаем… Это нормально! Главное вовремя встать и поехать к первой паре на Басмач… Ты хорошо постарался! Желаем, чтобы ридинги Даниэля действительно были попроще и полегче, а лмс никогда ложился… 
