...
Это бот с тестом-игрой «Насколько ты фиклёнок?», созданный для студентов 1 курса ФиКЛ.\n
Вот несколько команд, которые могут тебе помочь:\n
/start – собственно начинает игру\n
/next – если тебе наскучил какой-то раздел, можешь перейти к следующему
(но осторожно, обратно вернуться не получится!)\n
...
def next_message(next_m):
    ...
    if sections == 1:
        ...
        elif "Организация учебного процесса" in sections_check:
            keyboard_next_step = telebot.types.ReplyKeyboardMarkup(False, True)
            keyboard_next_step.row("Мемы")
            keyboard_next_step.row("Цитаты преподавателей")
            mesg = bot.send_message(next_m.chat.id, "Выбери следующий раздел:", reply_markup=keyboard_next_step)
            bot.register_next_step_handler(mesg, org_next_step1)
        elif "Цитаты преподавателей" in sections_check:
            keyboard_next_step = telebot.types.ReplyKeyboardMarkup(False, True)
            keyboard_next_step.row("Организация учебного процесса")
            keyboard_next_step.row("Мемы")
            mesg = bot.send_message(next_m.chat.id, "Выбери следующий раздел:", reply_markup=keyboard_next_step)
        ...
        if "Цитаты преподавателей" not in sections_check:
            keyboard_next_step = telebot.types.ReplyKeyboardMarkup(False, True)
            keyboard_next_step.row("Цитаты преподавателей")
            mesg = bot.send_message(next_m.chat.id, "Выбери следующий раздел:",
                                    reply_markup=keyboard_next_step)
            bot.register_next_step_handler(mesg, org_next_step1)
        elif "Мемы" not in sections_check:
            keyboard_next_step = telebot.types.ReplyKeyboardMarkup(False, True)
            keyboard_next_step.row("Мемы")
            mesg = bot.send_message(next_m.chat.id, "Выбери следующий раздел:",
                                    reply_markup=keyboard_next_step)
            bot.register_next_step_handler(mesg, org_next_step1)
        elif "Организация учебного процесса" not in sections_check:
            keyboard_next_step = telebot.types.ReplyKeyboardMarkup(False, True)
            keyboard_next_step.row("Организация учебного процесса")
            mesg = bot.send_message(next_m.chat.id, "Выбери следующий раздел:",
                                    reply_markup=keyboard_next_step)
...
связанный с нашим потоком. Вам нужно будет выбрать один вариант ответа или ввести 
слово.''', reply_markup=types.ReplyKeyboardRemove())
...
def organisation1(org1):
    ...
    sections_check.append('Организация учебного процесса')
    if org1.text == "/end":
...
def quote1(message):
    global sections_check
    sections_check.append('Цитаты преподавателей')
    if message.text == "/end":
        bot.send_message(message.chat.id, "Заканчиваем...")
        end_message(message)
